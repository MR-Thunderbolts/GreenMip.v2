  ...
  <Container class="flex flex-col gap-20">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
      ...
      <div class="flex justify-center w-full">
        <div class="rounded-3xl overflow-hidden border border-[#e5e7eb] aspect-[423/383] w-full">
          <img
        ...
        <div class="flex flex-col gap-2 mt-2 w-full">
          ...
          <div class="flex flex-wrap gap-2">
            {#each about.categories as category}
              <span class="font-krub font-semibold text-xs text-greenmip-sage border border-gray-300 rounded-full px-3 py-1.5">
                {category}
              </span>
            {/each}
          </div>
    ...
    <div class="flex flex-col gap-8 items-center mt-8">
        ...
        <Carousel itemCount={about.testimonials.length} dotVariant="on-light">
          {#snippet children()}
            {#each about.testimonials as item, i}
              <Card as="blockquote" variant="flat" class="border border-greenmip-sage p-8 flex flex-col gap-4 justify-between items-start shrink-0 w-[90%] snap-start">
      ...
      <div class="hidden lg:grid grid-cols-3 gap-6 items-stretch w-full">
        {#each about.testimonials as item}
          <Card as="blockquote" variant="flat" class="border border-greenmip-sage p-8 flex flex-col justify-between hover:shadow-md transition-all duration-300">
