  ...
  <div class="navbar bg-white shadow-lg rounded-2xl px-2 md:px-8 py-2 max-w-6xl mx-auto transition-all duration-300 flex-nowrap gap-1">
    ...
    <div class="navbar-end flex-none gap-1 md:gap-2">
      ...
      <a
        ...
        class="btn bg-greenmip-bright text-greenmip-forest hover:bg-greenmip-bright/80 border-none font-semibold px-3 md:px-6 py-2 rounded-lg min-h-[40px] h-[40px] text-xs md:text-sm tracking-wide shadow-sm hover:shadow-md transition-all duration-200"
      >
        {navbar.cta.label}
      </a>
      ...
      <div class="dropdown dropdown-end lg:hidden">
        ...
        <ul tabindex="0" class="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow-lg bg-white border border-greenmip-border rounded-box w-52 text-greenmip-forest font-semibold gap-1">
          {#each navbar.links as link}
            <li>
              <a
                href="#{link.id}"
                onclick={(e) => { e.preventDefault(); setActive(link.id); }}
                class="w-full text-left py-2 px-3 rounded-md flex items-center gap-2 hover:bg-greenmip-light-bg {activeSection === link.id ? 'font-bold' : ''}"
              >
                {#if activeSection === link.id}
